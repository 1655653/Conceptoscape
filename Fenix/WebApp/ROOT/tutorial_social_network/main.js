...
//   });
//   ///FUNZIONE ADDTOGRAPH CHIAMA twitterUserObjToCyEle SUL TARGET E SUI FOLLOWERS
//   function addToGraph(targetUser, followers, level) { //FOLLOWES È UN ARRAY
//     // targetUser
//     if (cy.getElementById(targetUser.id_str).empty()) {//SOLO SE NEL GRAFO CY NON C'È TWITTERFOLLOWER.ID_STR
...
//     cy.batch(function() { //FA FINIRE IL FOREACH E POI MODIFICA IL GRAFO (EFFICENTE)
//       followers.forEach(function(twitterFollower) {
//         if (cy.getElementById(twitterFollower.id_str).empty()) { //SOLO SE NEL GRAFO CY NON C'È TWITTERFOLLOWER.ID_STR
...
//     });
//   }
//   //FUNZIONE twitterUserObjToCyEle PARSA UN ELEMENTO DEL JSON
...
//       dataType: 'json'
//     });
//           ///promise.all ritorna una promise che viene data in pasto a .then()
//     return Promise.all([userPromise, followersPromise])
//       .then(function(then) { //il risultato di then è una promessa che viene data in pasto alla funzione
//         return {
//           user: then[0],
//           followers: then[1]
//         };
...
//   ///////////ADDFOLLOWERBY LEVEL E TUTTE LE UTILITY
//   function addFollowersByLevel(level, options) { //ricorsiva, ogni ricorsione aumenta il livello
//     function followerCompare(a, b) {
//       return a.data('followerCount') - b.data('followerCount');
//     }//>0 se a>b, <0 se a<b perchè a e b sono entrambi >0

//     function topFollowerPromises(sortedFollowers) {
//       return sortedFollowers.slice(-options.usersPerLevel)//prende gli elementi dell'array sortedfollower partendo dalla fine 
...
//         .then(function(userAndFollowerData) {///userAndFollowerData è una matrice #followerpromises*2(user+follower)
