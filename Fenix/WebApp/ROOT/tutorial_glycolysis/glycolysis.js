document.addEventListener("DOMContentLoaded", function() { 
    var cy = cytoscape({
      ...
      cy.autolock(true);
      ///////////////panIn serve per fare focus su target
      ...
      //////////trova il nodo successore a selected (se tocco nodo prende  quell uscente caso di DHAP prende max)
      function findSuccessor(selected) {
        if(selected.id() == 10) return cy.nodes('[molecule = "Glucose"]');
        var connectedNodes;
        if (selected.isEdge()) {
      ...
      var advanceButton = document.getElementById('AdvButt');
      advanceButton.addEventListener('click', function() {
        var previous = cy.$(':selected');
        advanceByButton(previous);
      });
      //reindirizza all'url del nodo
      ...
      var startNode = cy.$('node[molecule = "Glucose"]'); //graph.$(selector)
      startNode.select();
      panIn(startNode);
